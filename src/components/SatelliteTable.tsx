...
export default function SatelliteTable() {
  ...
  return (
    <div className="border border-white/10 relative hud-corner group">
      ...
      <div className="p-6 md:p-8 border-b border-white/10 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <span className="text-technical mb-2 block opacity-40">Orbital Assets</span>
          <h2 className="text-xl md:text-2xl font-medium text-cinematic uppercase tracking-tight text-white flex items-center">
            Integrated Catalog
          </h2>
        </div>
        ...
        <div className="flex items-center space-x-4">
          <div className="relative">
            ...
            <input
              ...
              className="bg-white/[0.03] border border-white/10 rounded-full pl-10 pr-6 py-2 text-[10px] text-white uppercase tracking-widest focus:outline-none focus:border-white/30 transition-all w- md:w-64 font-bold"
            />
          </div>
        ...
        <table className="w-full text-left font-sans min-w-[800px]">
          <thead>
            <tr className="bg-white/[0.02] text-white/30 text-[9px] uppercase tracking-[0.2em] font-bold">
              <th className="px-6 md:px-8 py-3 md:py-4">ID <ArrowUpDown className="inline h-2 w-2 ml-1" /></th>
              <th className="px-6 md:px-8 py-3 md:py-4">Designation</th>
              <th className="px-6 md:px-8 py-3 md:py-4">Classification</th>
              <th className="px-6 md:px-8 py-3 md:py-4">Lat/Lon</th>
              <th className="px-6 md:px-8 py-3 md:py-4">Alt (km)</th>
              <th className="px-6 md:px-8 py-3 md:py-4 text-right">Status</th>
            </tr>
          </thead>
          <tbody className="text-[11px] divide-y divide-white/5 font-mono">
            {paginatedSatellites.map((sat: SatelliteData) => (
              <tr 
                key={sat.norad_id} 
                className="hover:bg-white/[0.03] transition-colors cursor-pointer group/row"
              >
                <td className="px-6 md:px-8 py-4 md:py-5 text-white/40">{sat.norad_id}</td>
                <td className="px-6 md:px-8 py-4 md:py-5 text-white font-medium group-hover/row:text-white transition-colors">{sat.name}</td>
                <td className="px-6 md:px-8 py-4 md:py-5 text-white/40 uppercase tracking-tighter">{sat.type}</td>
                <td className="px-6 md:px-8 py-4 md:py-5 text-white/60">{sat.lat.toFixed(2)} / {sat.lon.toFixed(2)}</td>
                <td className="px-6 md:px-8 py-4 md:py-5 text-white/60">{sat.alt.toLocaleString()}</td>
